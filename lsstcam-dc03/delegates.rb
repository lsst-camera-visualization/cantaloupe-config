...
class CustomDelegate
  ...
  def filesystemsource_pathname(options = {})
      ...
      begin
         ...
         puts folder
         if tokens[4] == "all"
            tmpFile = fpFile + rand(1000000).to_s
            if options.length()>1
               File.write(tmpFile,"\#" + options[1, options.length()].join("\n\#")+"\n") 
            end
            system("ls -1 #{folder}/*.fits >> '#{tmpFile}'")
            File.rename(tmpFile,fpFile)
            return fpFile
         else
            tmpFile = raftFile + rand(1000000).to_s
            if options.length()>1
               File.write(tmpFile,"\#" + options[1, options.length()].join("\n\#")+"\n") 
            end
            puts "System: ls -1 #{folder}/*#{tokens[4]}*.fits >> '#{tmpFile}'"
            system("ls -1 #{folder}/*#{tokens[4]}*.fits >> '#{tmpFile}'")
            File.rename(tmpFile,raftFile)
            return raftFile
         end
      ensure
         conn.close()  
