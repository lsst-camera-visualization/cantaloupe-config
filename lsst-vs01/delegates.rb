...
class CustomDelegate
  ...
  def shared_connection_pool()
      if ($connection_pool == nil)
         ...
         $connection_pool.setJdbcUrl("jdbc:mysql://lsst-ir2db01.slac.stanford.edu:3306/ir2dbprod?serverTimezone=UTC&autoReconnect=true")
  ...
  def filesystemsource_pathname(options = {})
      ...
      identifier = context['identifier']
      imageRoot = "/home/tonyj/images"
      file = "#{imageRoot}/#{identifier}"
      if File.file?(file)
        puts file
        return file
      end
      ...
      raftFile = "#{imageRoot}/#{tokens[2]}/#{identifier}.raft"
      if File.file?(raftFile)
        puts raftFile
        return raftFile
      end
      fpFile = "#{imageRoot}/#{tokens[2]}/#{identifier}.fp"
      if File.file?(fpFile)
        puts fpFile
        return fpFile
      end
      cFile = "#{imageRoot}/#{tokens[2]}/#{identifier}.cantaloupe"
      if File.file?(cFile)
        puts cFile
        return cFile
      end
      if !Dir.exist?("#{imageRoot}/#{tokens[2]}")
         Dir.mkdir("#{imageRoot}/#{tokens[2]}")
      end
      conn = shared_connection_pool().getConnection()
      begin
         stmt = conn.prepareStatement("select fileLocation from ccs_image where telCode=? and controller=? and dayobs=? and seqnum=?")
         stmt.setString(1,tokens[0])
         stmt.setString(2,tokens[1])
         stmt.setString(3,tokens[2])
         stmt.setInt(4,tokens[3].to_i())
         ...
         if tokens[4] == "all"
            if options.length()>1
               File.write(fpFile,"\#" + options[1, options.length()].join("\n\#")+"\n") 
            end
            system("ls -1 #{folder}/*.fits >> '#{fpFile}'")
            return fpFile
         else
            if options.length()>1
               File.write(raftFile,"\#" + options[1, options.length()].join("\n\#")+"\n") 
            end
            system("ls -1 #{folder}/*#{tokens[4]}*.fits >> '#{raftFile}'")
            return raftFile
         end
      ensure
         conn.close()  
